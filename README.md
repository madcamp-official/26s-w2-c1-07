...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 맵 (구간 기반 레인) | 시드 결정론 로컬 생성(`ClimbMapGenerator`, 전 피어 동일·NetworkObject 없음). 레인 = 진입 발판 + **구간 체인 6유형** + 정상 발판: ① **계단형**(쿠션 스텝, 전진 1.5~2.1/상승 0.5~0.7) ② **지그재그형**(쿠션 스텝, 레인 중심선 좌우 교대 ±1.3~1.7) ③ **수직 샤프트형**(바위 스텝, 상승 0.75~0.88, 풀홀드 점프 요구) ④ **가구 방형**(공중 가구 사다리, 윗면 간 0.9씩: 쿠션→테이블→테이블→냉장고 — 바닥 슬래브 없음) ⑤ **무너진 다리형**(넓적 바위 돌다리 + 단절 갭 4.8~5.6 — 구조물 설치 필수, 레인당 1~2회 강제) ⑥ **회전 교차로형**(인접 레인 쌍 60%: 바위 디딤돌로 합류하는 회전 십자 — 눕힌 서랍장 2개를 팔 길이로 늘려 교차, 서버시간 순수함수 회전 + 시드 위상). 구간은 상승 예산(`targetRisePerLane` 10m)까지 이어붙이고 정상 입구는 정확히 +0.5 — 전 레인 top 동일(공정성). 구간 사이 휴게 발판은 풀(가구 8 + 자연물 8, 가중치)에서 선택하며, 낮은 확률(`tiltChance` 12%)로 90도 회전(눕힘/세움)된 형태로 나온다(앵커·콜라이더는 회전된 바운즈로 재계산). **모든 발판은 에셋 기반**(유니티 기본 도형 발판 없음, 조각 프리팹 미배선 시에만 슬래브 폴백): 구간 조각은 `stepPiece/shaftPiece/stonePiece/bridgePiece/junctionArmPiece` 필드로 교체 가능. 지오메트리 `Ground` 레이어 + 투명 경계벽 + **도착선**(정상 높이 반투명 핑크 평면 `M_FinishLine`, 판정과 1:1) 유지. 랜덤 소비 규율: 레인 지오메트리 = 레인 파생 rng, 교차로 = 별도 파생 시드, 디딤돌 = rng 미사용(순수 기하). 기존 균일 체인은 `useSectionLanes=false` 폴백. 검증: 시드 8종 전수 감사(점프 포락선·단절 수·top 일치) FAIL 0. | 필수 |
