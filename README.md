...
> Day 1(기획·셋업·웹 프로토로 설계 검증·Unity 전환)은 완료. 아래는 Unity 빌드 기준 Day 2~7.

| 날짜 | 목표 |
|---|---|
| Day 2 | NGO + Steam(Facepunch) 접속·이동 동기화 (친구와 같은 씬에서 서로 보임) |
| Day 3 | Terrain(산·강) + CharacterController 이동/점프 + Cinemachine, 강물 낙사 |
| Day 4 | 매치 FSM(NetworkBehaviour) + 게임 모드 3종(레이스/높이/생존) + 사망/부활 |
| Day 5 | 준비 페이즈(장애물 배치·투표·룰렛) + 점수 산식 이식 |
| Day 6 | UI 전체 + 3라운드 통합 + 하이라이트 + 다인원 플레이테스트 |
| Day 7 | 폴리싱·버그 수정·Windows 빌드·시연 자료·회고 |
