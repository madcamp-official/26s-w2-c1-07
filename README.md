...
| 구조물 2종(보임/안보임) | 보이는 구조물(형태 4종: 벽·원기둥·나무·바위): 생성 후 모든 페이즈에서 전원에게 보임. **형태는 설치할 때마다 랜덤으로 굴려진다**(PrepClientUI 가 클라 로컬로 굴림 — 결과물은 서버 스폰·복제라 전 피어 일치 불필요, 블루프린트 프리뷰는 굴려진 형태와 1:1). 키: [1] 보이는(랜덤) / [2] 투명. 보이지 않는 구조물: PREP 에는 설치자에게만(반투명), 그 외 페이즈엔 아무에게도 안 보임, 플레이어 충돌 시 `revealDuration`(2초) 동안 전원 공개. 콜라이더는 항상 전원 동일(공정). | 필수 |
