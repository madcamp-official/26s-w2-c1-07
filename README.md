...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 장애물 배치 (ServerRpc + NetworkObject 스폰) | 커서 잠금 상태에서 화면 중앙 조준점으로 배치: 숫자키 1/2/3 으로 종류 선택 후 좌클릭 → 카메라 중앙 레이캐스트 히트점을 `PlaceObstacleServerRpc(pos,type)`로 요청 → 호스트가 검증(PREP 한정·1인 3개·코스 구역·강 금지·겹침) 후 장애물 프리팹을 `NetworkObject.Spawn()` → 전 클라 자동 복제. 검증 실패는 조용히 거부. | 필수 |
| 장애물 3종 프리팹 | 벽·원기둥·투명벽. 투명벽은 동일 콜라이더 + 렌더러만 분기(설치자에게만 반투명 표시, 타인에겐 비가시. 모두 같은 물리라 공정, 시각만 기만). | 필수 |
| 주제 투표 | 숫자키 4/5/6 으로 `VoteServerRpc(topic)` → 호스트 집계(주제별 `NetworkVariable`) → PREP 패널·HUD 에 표수 실시간 반영. 1인 1표(재투표 시 덮어씀). | 필수 |
| 투표율 가중 룰렛 | 호스트가 가중치(투표수+1, 직전 주제 제외)로 결과 결정 → `Topic` NetworkVariable 복제. 클라는 HUD 에서 감속 룰렛 연출을 재생하고 실제 결정된 주제에 정착(공정성 보장). | 필수 |
...
| 사망/부활/관전 | 슬라임 바다 낙사(기준 y 아래) 판정 → 부활(레이스/높이) 또는 관전(생존). 호스트 권위. | 필수 |
...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 사망 이벤트 로깅 | 호스트가 라운드 사망(위치·시각)을 판정하고, 낙사 지점 반경 내 타인 소유 장애물에 kill 을 귀속(장애물 점수의 원천). | 필수 |
| 라운드 결과 카드 | 하이라이트 페이즈에 라운드 우승자·주제·Top3 카드 표시(복제된 `Results` 기반, 클라 로컬 렌더). | 필수 |
| 임팩트 구간 스탯 카드 | 슬라이딩 윈도우로 최다 낙사 구간·최다 유발 장애물 산정 → 카드 표시. | 권장 |
...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 맵 (캔디 코스) | 폴가이즈풍 플로팅 캔디 코스(씬 배치 정적 지오메트리): 파스텔 플랫폼 징검다리·경사로·하이 덱·좁은 다리 3열·나선 타워(높이 모드용, 최고 12)·슬라롬 벽·시작/골 게이트·핑크 슬라임 바다. 맵 콜라이더는 `Ground` 레이어. 스폰 y·장애물 배치 검증은 `MapSurface`(레이캐스트)가 담당해 맵을 교체해도 코드 수정이 없다. | 필수 |
...
| UI (uGUI 코드 생성) | 프리팹 없는 코드 생성 HUD: 페이즈 배너·타이머·누적 점수판·룰렛 연출·플레이 목표·하이라이트 카드·최종 결과·조준점·머리 위 이름표. PREP 은 키 안내 정보 패널. | 필수 |
...
```
 [Unity 클라이언트 x N]                          [호스트 (서버+클라 겸용)]
  ...
  (맵·장애물·캐릭터)        <-NetworkVariable/RPC-  페이즈·점수·룰렛·사망 판정 (권위)
...
페이즈 구동 단일 화면(3D 씬 위 UI 오버레이, 전 페이즈 마우스룩 조준 시점·커서 잠금). 로비(접속) → 준비(숫자키 1/2/3 장애물 선택, 화면 중앙 조준점에 좌클릭 배치, 4/5/6 주제 투표) → 룰렛(감속 추첨 연출) → 플레이(상단 페이즈·타이머 배너, 목표 문구, 조준점) → 하이라이트(라운드 우승자·Top3 카드) → 결과(전체 화면 최종 순위). 우상단에 누적 점수판 상시 표시, Esc 로 커서 잠금 해제.
...
| 종류 | 이름 | 방향 | 설명 |
|---|---|---|---|
| ServerRpc | `PlaceObstacleServerRpc(pos,type)` | C→H | 장애물 배치 요청(호스트가 검증 후 스폰) |
| ServerRpc | `VoteServerRpc(topic)` | C→H | 주제 투표(1인 1표, 재투표 덮어씀) |
| Rpc(Single) | `TeleportPlayerRpc(pos)` | H→소유 클라 | 출발선 배치·부활(소유자가 스스로 텔레포트) |
| NetworkVariable | `Phase / Round / Topic / PhaseEndTime / AliveCount / RoundWinner / MatchWinner / 주제별 Votes` | H→C | 페이즈·타이머·투표·승자 동기화 |
| NetworkList | `Results` (라운드별 순위·점수 행) | H→C | 점수판·하이라이트·결과 UI 데이터 |
| NetworkVariable | 장애물 `Type / OwnerId` (Obstacle) | H→C | 장애물 종류·설치자 동기화(스폰 페이로드 포함) |
| NetworkTransform | 플레이어 위치 | 소유 클라→전체 | ClientNetworkTransform(소유자 권위) 보간 |

(C=클라이언트, H=호스트)

낙사는 별도 보고 RPC 없이 호스트가 복제된 위치(y)를 읽어 직접 판정한다. 룰렛 연출과 하이라이트는 복제 상태(`Topic`, `Results`)를 바탕으로 각 클라가 로컬 렌더링한다.
...
3. 다른 인스턴스(Multiplayer Play Mode 가상 플레이어 또는 빌드 실행)에서 호스트 IP 입력 후 [Client]
   (같은 PC 는 127.0.0.1, 같은 네트워크는 호스트 사설 IP)
