...
| 사운드 (`AudioManager`) | 전역 사운드 매니저(`Resources/NetworkRig` 프리팹에 상주, DontDestroyOnLoad). SFX 9종(점프·착지·설치·투명 발동·탈락·라운드 시작·종료·최종결과·**UI 클릭**) + 로비/플레이 BGM 크로스페이드 슬롯. `AudioManager.Play(Sfx.X)` 정적 호출 한 줄(인스턴스/클립 없으면 무음), 라운드 스팅어·BGM 은 매니저가 페이즈를 폴링해 자동 처리. 볼륨은 `SettingsManager`(마스터=AudioListener, SFX/BGM 개별) 연동. **음원 = Kenney(CC0, 출시 안전)**: `Assets/Audio/SFX/` 에 슬롯당 1클립 선별 임포트(팩 전체가 아니라 사용분만 - 리포 경량). 캐주얼 톤으로 선정 - 점프=digital `pepSound`(어택 빠른 경쾌한 hop), 착지=impact `impactGeneric_light`, 설치=impact `impactPlank`(나무 쿵), 발동=interface `bong`(짧은 '땅!'), 탈락=digital `lowThreeTone`(코믹 하강 3톤), 시작/종료/결과=music-jingles `Pizzicato/Steel/Sax`, UI 클릭=interface `click`(0.1s 경쾌). **UI 버튼 클릭음**: LobbyUI·SettingsManager 의 모든 IMGUI 버튼을 `Clk()` 래퍼로 감싸 클릭 프레임에 `Sfx.UIClick` 재생. 트리거는 게임플레이 코드에 배선 완료(설치·발동·탈락은 전 클라 재생). BGM 은 미도입(무음 슬롯 유지). 확장 후보: 자주 나는 소리 랜덤 배리에이션, 3D 공간 음향 + 전원 재생, AudioMixer 그룹·더킹. | 권장(구현됨) |
