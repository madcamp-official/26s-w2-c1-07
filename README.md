...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 구조물 설치 (ServerRpc + NetworkObject 스폰) | PREP 동안 자유 비행 고스트 카메라로 맵 볼륨을 날며 조준점에 좌클릭 설치. **배치 큐 = 카트라이더식 아이템 슬롯 바(좌상단)**: PREP 시작 시 이번 라운드 지급량 전체(보이는 N + 투명 M)를 미리 굴려 **지금 설치할 것 = 큰 노란 슬롯, 이어질 순서 = 작은 크림 슬롯**으로 실물 썸네일(런타임 렌더) 표시. **[Alt]** 전환, **[1]/[2]** 보이는/투명 점프. 화면에는 슬롯 바와 "[Alt] 전환 · [F1] 도움말" 힌트 한 줄만 남기고, **조작 상세 설명은 전부 F1 도움말(설정 - 설명 탭)로 이동**. 회전은 3축 90도 스텝: **[R]** Y축, **[T]** X축, **[G]** Z축(블루프린트·실물 모두 회전된 바운즈로 바닥 정렬). `PlaceStructureServerRpc(pos,yaw,pitch,roll,type,sizeTier)` → 호스트 검증(PREP 한정·종류별 잔여 개수·볼륨 경계·겹침, 크기 배율은 서버 배열 기준) 후 스폰. | 필수 |
| 구조물 2종(보임/안보임) | 보이는 구조물(형태 4종: **테이블**·원기둥·나무·바위 - 벽은 타입/프리팹 보존, 풀에서 제외): 생성 후 모든 페이즈에서 전원에게 보임. **크기 티어**: 배치 큐가 낮은 확률로 중(15%)/대(7%)를 굴려 스케일 배율(소 1/중 1.5/대 2, `MatchManager._structureSizeMultipliers`) 적용 - 슬롯에 중/대 배지 표시, 블루프린트와 실물이 같은 배율(투명 구조물은 항상 소, 함정 밸런스). **형태는 PREP 시작 시 지급량 전체를 미리 굴려 배치 큐로 확정**(PrepClientUI 가 클라 로컬로 굴림 — 결과물은 서버 스폰·복제라 전 피어 일치 불필요, 블루프린트 프리뷰는 큐의 선택 형태와 1:1). 보이지 않는 구조물: PREP 에는 설치자에게만(반투명), 그 외 페이즈엔 아무에게도 안 보임, 플레이어 충돌 시 `revealDuration`(2초) 동안 전원 공개. 콜라이더는 항상 전원 동일(공정). | 필수 |
