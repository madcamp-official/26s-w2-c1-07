...
| 배경·분위기 | **커스텀 그라데이션 스카이박스**(`RouletteParty/SkyGradient` 셰이더 + `M_Sky`, 두 씬 공용): 상/지평선/하 3색을 방향 벡터로 보간하는 스타일라이즈드 하늘. 대형 구 방식 대신 스카이박스를 쓴 이유 = 깊이 무한대(far plane 에 안 잘림)·컬링/추적 불필요·포그와 자연스러운 혼합(셰이더 주석에 기록). 하늘 입체감은 **저폴리 구름**(`Prefabs/Env/Cloud`, 눌린 구 3개 뭉침, 콜라이더 없음/그림자 끔)을 로비 4개·게임 7개 배치해 살린다. 선형 포그(로비 35~120m / 게임 60~220m) + 스카이박스 앰비언트(1.15). 대기방 씬은 우드 톤 스테이지(`M_LobbyStage`)에 나무/버섯/바위 장식으로 타이틀 배경을 겸한다. | 필수(구현됨) |
