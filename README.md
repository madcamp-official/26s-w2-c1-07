...
| 플레이어 잡기 (좌클릭 꾹, `PlayerGrab`) | 등반(PLAY) 전용, 폴가이즈 식 서버 검증. 지상에서 좌클릭 유지 시 전방 1.7m·65° 이내 생존 상대를 붙잡는다: 피해자 이동 35%·점프 봉인(Grabbed = NetworkVariable, 서버 write), 잡는 쪽도 60% 감속(리스크). 최대 지속 2초·쿨다운 3초·거리 이탈·탈락·페이즈 종료 해제는 서버 틱이 강제. (렛지/사물 잡기는 검토 후 제외 — 2026-07-13) | 필수(구현됨) |
