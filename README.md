...
| 카메라 (마우스룩 조준) | 전 페이즈 배그식 3인칭 오버숄더 마우스룩: 마우스=조준(화면 중앙 조준점), WASD 는 카메라 기준 이동, 몸은 카메라 방향. SphereCast 오클루전 + 지형 높이 클램프, 커서 잠금(Esc 토글). 후방추적 시점은 `useAimView=false` 로 보존. | 필수 |
