...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 매치 상태머신 (NetworkBehaviour) | 호스트의 `MatchManager : NetworkBehaviour` 가 FSM(LOBBY → [PREP → PLAY → HIGHLIGHT] ×3 → RESULT) 구동. 룰렛/주제 없음. `NetworkVariable<Phase>` + 종료 타임스탬프로 전 클라 동기화. | 필수 |
...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 맵 (랜덤 타워) | 6×6×50 볼륨 랜덤 생성 클라이밍 타워(`ClimbMapGenerator`): 시드 기반 결정론 생성으로 전 피어 동일, 슬라이스(75등분)당 보이는 구조물 2개, 바닥 6×6 + 투명 경계벽. 지오메트리는 `Ground` 레이어(카메라 오클루전·스폰 판정 `MapSurface` 호환), 캔디 파스텔 머티리얼 재사용. 점프력 Δy=1 기준 밸런싱. | 필수 |
