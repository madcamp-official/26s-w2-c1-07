...
| 잡기 (좌클릭 꾹, `PlayerGrab`) | 등반(PLAY) 전용 2종. **렛지 잡기**(Only Up 식, 소유자 로컬): 공중에서 좌클릭 유지 시 전방 모서리 감지(윗면 프로브 -> 벽면 보정 레이, 발끝 기준 +0.4~2.2m, 리치 0.8m) -> 매달림 -> Space 올라서기 / 해제·S 낙하. 억지 잡기 방지 게이트: 하강 중에만 + 공중 0.12s 경과 후 + 벽을 55° 이내로 마주볼 때만 잡히고, 진입은 0.12s 블렌드(순간 스냅 아님). 매달림은 중력 정지·움직이는 발판 추적, 숨겨진 투명 구조물은 잡기 불가. **플레이어 잡기**(폴가이즈 식, 서버 검증): 지상에서 전방 1.7m 상대를 붙잡아 이동 35%·점프 봉인, 잡는 쪽도 60% 감속. 최대 2초·쿨다운 3초·거리 이탈 해제는 서버 강제, 매달린 상대를 잡으면 떨어뜨림. | 필수(구현됨) |
