...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 맵 (구간 기반 레인) | 시드 결정론 로컬 생성(`ClimbMapGenerator`, 전 피어 동일·NetworkObject 없음). 레인 = 진입 발판 + **구간 체인 6유형** + 정상 발판: ① **계단형**(짧고 안정적 연속 점프, 전진 1.5~2.1/상승 0.5~0.7) ② **지그재그형**(레인 중심선 좌우 교대 ±1.3~1.7) ③ **수직 샤프트형**(좁은 공간 큰 상승 0.75~0.88, 풀홀드 점프 요구) ④ **가구 방형**(바닥 7×7 + 가구 사다리, 윗면 간 0.9씩 천장까지: 쿠션→테이블→테이블→냉장고) ⑤ **무너진 다리형**(플랭크 + 단절 갭 4.8~5.6 — 구조물 설치 필수, 레인당 1~2회 강제) ⑥ **회전 교차로형**(인접 레인 쌍 60%: 두 레인이 디딤돌로 합류하는 회전 십자 발판, 서버시간 순수함수 회전 + 시드 위상). 구간은 상승 예산(`targetRisePerLane` 10m)까지 이어붙이고 정상 입구는 정확히 +0.5 — 전 레인 top 동일(공정성). 구간 사이 휴게 발판은 풀(가구 8 + 자연물 8, 가중치)에서 선택. 슬래브는 캔디 파스텔 순환. 지오메트리 `Ground` 레이어 + 투명 경계벽 + **도착선**(정상 높이 반투명 핑크 평면 `M_FinishLine`, 판정과 1:1) 유지. 랜덤 소비 규율: 레인 지오메트리 = 레인 파생 rng, 교차로 = 별도 파생 시드, 디딤돌 = rng 미사용(순수 기하). 기존 균일 체인은 `useSectionLanes=false` 폴백. 검증: 시드 8종 전수 감사(점프 포락선·단절 수·top 일치) FAIL 0. | 필수 |
