...
| 구현 요소 | 설명 | 우선순위 |
|---|---|---|
| 맵 (구간 기반 레인) | 시드 결정론 로컬 생성(`ClimbMapGenerator`, 전 피어 동일·NetworkObject 없음). 레인 = 진입 발판 + **구간 체인 6유형** + 정상 발판: 발판 에셋은 **크기 티어 소/중/대**로 분리 관리(`Assets/Prefabs/Platforms/{Small,Medium,Large}` 폴더 = 생성기 `smallPlatforms/mediumPlatforms/largePlatforms` 배열과 1:1, 소 4/중 12/대 4종): ① **계단형**(발판마다 소 티어 랜덤, 전진 1.5~2.1/상승 0.5~0.7) ② **지그재그형**(발판마다 소 티어 랜덤, 레인 중심선 좌우 교대 ±1.3~1.7) ③ **수직 샤프트형**(소/중 티어 혼합 — 중 확률 `shaftMediumChance` 35%, 상승 0.75~0.88 풀홀드 점프 요구, 확률적 미등반 허용) ④ **가구 방형**(랜덤 구성 사다리: 소2→중2→대1, 윗면 간 `roomStepRise` 0.9씩 상승, XZ 박스 관통 `roomOverlapTolerance` 0.3 초과 시 위치 재시도) ⑤ **무너진 다리형**(넓적 바위 돌다리 + 단절 갭 4.8~5.6 — 구조물 설치 필수, 레인당 1~2회 강제 보증) ⑥ **회전형**(자전하는 대 티어 발판 `spinCount` 3개 연속, `spinSpeed` 35도/초 서버시간 순수함수 + 시드 위상, 이웃 스윕 원 간 `spinGapMargin` 0.7 로 회전 중 상호 충돌 없음). 진입/휴게 발판 = 중 티어, 낮은 확률(`tiltChance` 12%)로 90도 회전(눕힘/세움) 등장. **부채꼴 레인 배치**: 레인들은 시작 청크에서 `fanAnglePerLane`(18도) 간격의 살(방사선)로 뻗고, **청크 배열(유형 순서·步 수·다리 갭)과 청크 경계 반경은 맵 시드에서 한 번 뽑아 전 레인 공유** — 레인마다 같은 순서의 청크가 같은 간격(경계 반경 ±0.3 이내 실측 일치)으로 나오되, 구성 에셋·상승·측면 배치는 레인별 랜덤이다. 청크가 밴드보다 짧게 끝나면 소 티어 디딤돌(Link, `linkStepMax`)로 경계까지 잇고 휴게 발판을 정확히 경계에 놓는다. 청크는 **레인당 `chunksPerLane`(5)개 고정**(상승 예산 정책 폐지 — 맵 높이는 뽑힌 청크 구성에서 자연히 결정, 실측 12~22m 분포)이며 **레인 top 은 제각각**(다양성 우선). 레인 구조: **시작 청크**(시작 섬) → 청크들 → **도착 청크**(진행 방향 정렬 큰 발판 `finishChunkSize` 10x10 + 반투명 도달 볼륨). **도달 판정은 높이가 아니라 도착 청크 위에 올라서는 것**(`IsAtFinish`: 발끝이 판 위 `finishDetectHeight` 2.5m 안) — 낮은 레인도 완주하면 만점(점수 정규화 기준은 가장 높은 도착 청크). 대각 진행 시 단절 갭이 모서리 지름길로 줄어드는 것은 실측 보정으로 차단. **모든 발판은 에셋 기반**(티어 배열 미배선 시에만 기본 도형 슬래브 폴백). 지오메트리 `Ground` 레이어 + 투명 경계벽 + **도착선**(정상 높이 반투명 핑크 평면 `M_FinishLine`, 판정과 1:1) 유지. 랜덤 소비 규율: 레인 지오메트리 = 레인 파생 rng, 교차로 = 별도 파생 시드, 디딤돌 = rng 미사용(순수 기하). 기존 균일 체인은 `useSectionLanes=false` 폴백. 검증: 시드 8종 전수 감사(점프 포락선·단절 수·top 일치) FAIL 0. | 필수 |
