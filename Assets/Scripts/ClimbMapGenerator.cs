...
///
///  - 맵 볼륨: mapWidth x mapDepth x mapHeight (기본 6 x 6 x 50). 바닥 + 투명 경계벽은 Awake 에 1회 생성.
///  - 랜덤 구조물: 볼륨을 높이 기준 sliceCount(x) 등분하고, 각 슬라이스마다 structuresPerSlice(y)개의
...
///  - 지오메트리는 전부 Ground 레이어(스폰 판정 MapSurface·카메라 오클루전 호환).
///
...
public class ClimbMapGenerator : MonoBehaviour
{
    ...
    [Header("맵 볼륨 (docs/클라이밍_전환_명세서.md 3절)")]
    ...
    void Update()
    {
        ...
        var mm = RouletteParty.Match.MatchManager.Instance;
    ...
    /// <summary>월드 좌표가 맵 볼륨 안인지(설치 검증용, 서버·클라 공용).</summary>
    ...
    void BuildStatic()
    {
        ...
        floor.transform.localScale = new Vector3(mapWidth, 1f, mapDepth);
        ...
        float t = 0.5f;
        BuildWall("Wall_XPos", new Vector3( mapWidth * 0.5f + t * 0.5f, h * 0.5f - 1f, 0f), new Vector3(t, h, mapDepth + 2f * t), ground);
        BuildWall("Wall_XNeg", new Vector3(-mapWidth * 0.5f - t * 0.5f, h * 0.5f - 1f, 0f), new Vector3(t, h, mapDepth + 2f * t), ground);
        BuildWall("Wall_ZPos", new Vector3(0f, h * 0.5f - 1f,  mapDepth * 0.5f + t * 0.5f), new Vector3(mapWidth + 2f * t, h, t), ground);
        BuildWall("Wall_ZNeg", new Vector3(0f, h * 0.5f - 1f, -mapDepth * 0.5f - t * 0.5f), new Vector3(mapWidth + 2f * t, h, t), ground);
    }
